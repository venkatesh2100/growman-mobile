...
export default function HomeScreen() {
  ...
  const renderCategoryCard = (category: Category, index: number) => (
    <Animated.View
      ...
      <TouchableOpacity
        ...
        <Text className="text-[13px] font-semibold text-gray-900 text-center" numberOfLines={1}>
          {category.name}
        </Text>
      </TouchableOpacity>
    </Animated.View>
  ...
  return (
    <ScrollView className="flex-1 bg-gray-50" showsVerticalScrollIndicator={false}>
      {/* Hero Section */}
      <Animated.View entering={FadeInUp.duration(500)} className="bg-green-600 pt-[60px] pb-10 px-5 rounded-b-[30px]">
        <View style={{ maxWidth: width - 40 }}>
          <Text className="text-[36px] font-bold text-white mb-4 leading-[44px]">
            Bring Nature{'\n'}Into Your <Text className="text-green-100">Home</Text>
          </Text>
          ...
          <TouchableOpacity
            ...
            <Text className="text-base font-semibold text-green-600">Shop Plants</Text>
      ...
      {categories.length > 0 && (
        <View className="mt-8 px-4">
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-[22px] font-bold text-gray-900">Shop by Category</Text>
            <TouchableOpacity onPress={() => router.push('/(tabs)/shop')}>
              <Text className="text-sm text-green-600 font-semibold">See All</Text>
            </TouchableOpacity>
          </View>
          <ScrollView
            horizontal
      ...
      <View className="mt-8 px-4">
        <View className="flex-row justify-between items-center mb-4">
          <Text className="text-[22px] font-bold text-gray-900">Featured Plants</Text>
          <TouchableOpacity onPress={() => router.push('/(tabs)/shop')}>
            <Text className="text-sm text-green-600 font-semibold">See All</Text>
          </TouchableOpacity>
        </View>
        ...
        {loading ? (
          ...
          <ScrollView
            ...
            {featuredProducts.map((product, index) => (
              <View key={product.id} className="w-[180px] mr-3">
                <ProductCard
                  product={product}
                  onPress={() => router.push({ pathname: '/product/[id]', params: { id: product.slug.toString() } })}
                  index={index}
                />
              </View>
            ))}
          ...
          <View className="py-10 items-center">
            ...
            <Text className="text-base text-gray-500 mt-3">No featured products available</Text>
          </View>
        )}
      </View>
